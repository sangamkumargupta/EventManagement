...
try {
    ...
    //Recipients
    $mail->setFrom('from@example.com', 'SS Event Management');
    #mail send..
    if(!empty($_REQUEST['submit']) && $_REQUEST['submit']=="submit" ){  
      if($getEmails == null){
        ...
        if($result){
          ...
          $mail->addAddress($_REQUEST['registerEmail']);
            $mail->isHTML(true);                                  
            $mail->Subject = 'For Registerd';
            $msg='<head>
            ...
            <p>Dear '.$_REQUEST['fname'].'   '.$_REQUEST['lname'].',</p>

            <p>Thank you so much for registered in my Application.<br/>
            Your most wel-come in SS Event Management.</p>
            
            <p>Thank you,<p>
            <h3>'.$_REQUEST['fname'].'   '.$_REQUEST['lname'].'</h3>
            </body>
            </html>';
